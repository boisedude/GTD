...
function applyTaskFilters(tasks: Task[], filters: TaskFilter): Task[] {
  return tasks.filter((task) => {
    ...
    if (filters.context && filters.context.length > 0) {
      if (!task.context || !filters.context.includes(task.context))
        return false;
    }
    ...
    if (filters.energy_level && filters.energy_level.length > 0) {
      if (
        !task.energy_level ||
        !filters.energy_level.includes(task.energy_level)
      )
        return false;
    }
    ...
    if (filters.estimated_duration && filters.estimated_duration.length > 0) {
      if (
        ...
        !filters.estimated_duration.includes(task.estimated_duration)
      )
        return false;
    }
...
export function useEngagement(
  ...
  const suggestions = useMemo(() => {
    const actionableTasks = tasks.filter(
      (task) => task.status === "next_action" || task.status === "project"
    );
  ...
  const updateContext = useCallback(
    (newContext: Partial<EngagementContext>) => {
    ...
    async (taskId: string, action: TaskAction) => {
      ...
      try {
        switch (action.type) {
          ...
          case "update":
            if (action.data) {
              await updateTask(taskId, action.data as Record<string, unknown>);
            }
            break;
