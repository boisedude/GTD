...
interface UseReviewsReturn {
  ...
  completeReviewStep: (stepId: string, data?: Record<string, unknown>) => Promise<void>
...
export function useReviews(options: UseReviewsOptions = {}): UseReviewsReturn {
  ...
  // Computed properties
  const isReviewing = currentSession?.status === 'active' || currentSession?.status === 'paused'

  // Start a new review session
  const startReview = useCallback(async (type: ReviewType): Promise<ReviewSession> => {
    try {
      setError(null)
      setLoading(true)
  ...
  const completeReviewStep = useCallback(async (stepId: string, data?: Record<string, unknown>): Promise<void> => {
    if (!currentSession) return
  ...
  const completeReview = useCallback(async (notes?: string): Promise<Review> => {
    ...
    try {
      ...
      const reviewData = {
        ...
        duration_minutes: Math.round(
          (new Date().getTime() - new Date(currentSession.started_at).getTime()) / 60000
        ),
  ...
  const loadDailyReviewData = useCallback(async (): Promise<DailyReviewData> => {
    try {
      setLoading(true)
      ...
      const data: DailyReviewData = {
        ...
        overdueTasks: tasks?.filter(t => t.due_date && t.due_date < today) || [],
      ...
      setError(err instanceof Error ? err.message : 'Failed to load daily review data')
      throw err
    } finally {
      setLoading(false)
    }
  ...
  const loadWeeklyReviewData = useCallback(async (): Promise<WeeklyReviewData> => {
    try {
      setLoading(true)
      ...
      setError(err instanceof Error ? err.message : 'Failed to load weekly review data')
      throw err
    } finally {
      setLoading(false)
    }
  ...
  // Load metrics
  const loadMetrics = useCallback(async (days: number = 30): Promise<void> => {
    try {
  ...
  const updateMetrics = useCallback(async (reviewType: ReviewType): Promise<void> => {
    try {
      const today = new Date().toISOString().split('T')[0]
      ...
      const updateData = existingMetrics ? {
        ...existingMetrics,
        [`${reviewType}_reviews_completed`]: (existingMetrics[`${reviewType}_reviews_completed`] || 0) + 1,
        updated_at: new Date().toISOString()
      } : {
        date: today,
        tasks_completed: 0,
  ...
  const getReviewStreak = useCallback((): number => {
    ...
    const dailyReviews = recentReviews.filter(r => r.type === 'daily')
  ...
  const getCompletionRate = useCallback((days: number = 7): number => {
    ...
    const totalCompleted = relevantMetrics.reduce((sum, m) =>
      sum + (m.daily_reviews_completed || 0), 0)
        ...
        (payload) => {
          if (payload.eventType === 'UPDATE' && payload.new) {
            setCurrentSession(prev =>
              prev?.id === payload.new.id ? payload.new as ReviewSession : prev
            )
          }
        }
      )
