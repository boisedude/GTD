...
export function useOfflineActions(options: UseOfflineActionsOptions = {}): UseOfflineActionsReturn {
  const {
    maxRetries = 3,
    retryDelay = 1000,
    onSync,
  ...
  const executeAction = useCallback(async (offlineAction: OfflineAction): Promise<void> => {
    try {
      ...
      setPendingActions(prev => prev.filter(a => a.id !== offlineAction.id))
  ...
  const queueAction = useCallback((taskId: string, action: TaskAction): string => {
