...
export function useTimer(options: UseTimerOptions = {}): UseTimerReturn {
  ...
  // Start timer
  const start = useCallback((taskId: string, targetMinutes?: number) => {
    if (isRunning) return
  ...
  // Stop timer
  const stop = useCallback((notes?: string) => {
  ...
  useEffect(() => {
    if (autoSave) {
      try {
        ...
        console.warn('Failed to restore timer session from localStorage:', error)
        localStorage.removeItem('currentTimerSession')
      }
    }
  }, [autoSave])
