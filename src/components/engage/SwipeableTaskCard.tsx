...
export function SwipeableTaskCard({
  ...
  const [activeAction, setActiveAction] = useState<
    keyof typeof swipeActions | null
  >(null);
  ...
  useEffect(() => {
    ...
    const handleMouseUp = async () => {
      ...
      if (
        activeAction &&
        Math.abs(swipeOffset) >= swipeActions[activeAction].threshold
      ) {
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      ...
      <div className="absolute inset-0 flex">
        {/* Left action (defer) */}
        <div
          className={`${swipeActions.defer.color} flex items-center justify-start pl-4 transition-opacity duration-200 ${swipeOffset < -swipeActions.defer.threshold ? "opacity-100" : "opacity-60"}`}
        >
          <swipeActions.defer.icon className="h-6 w-6 text-white mr-2" />
          <span className="text-white font-medium">
            {swipeActions.defer.text}
          </span>
        </div>
        ...
        <div className="flex ml-auto">
          {/* Complete action */}
          <div
            className={`${swipeActions.complete.color} flex items-center justify-center px-4 transition-opacity duration-200 ${swipeOffset >= swipeActions.complete.threshold && swipeOffset < swipeActions.delegate.threshold ? "opacity-100" : "opacity-60"}`}
          >
            <swipeActions.complete.icon className="h-6 w-6 text-white mr-2" />
            <span className="text-white font-medium">
              {swipeActions.complete.text}
            </span>
          </div>

          {/* Delegate action */}
          <div
            className={`${swipeActions.delegate.color} flex items-center justify-center px-4 transition-opacity duration-200 ${swipeOffset >= swipeActions.delegate.threshold ? "opacity-100" : "opacity-60"}`}
          >
            <swipeActions.delegate.icon className="h-6 w-6 text-white mr-2" />
            <span className="text-white font-medium">
              {swipeActions.delegate.text}
            </span>
          </div>
        </div>
      </div>
