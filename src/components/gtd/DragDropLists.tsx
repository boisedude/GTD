...
export function DragDropLists({ onTaskEdit, onTaskCreate, className }: DragDropListsProps) {
  ...
  const handleDragOver = useCallback((e: React.DragEvent, listId: TaskStatus) => {
    e.preventDefault()
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-600">
            Drag tasks between lists to organize them. {draggedTask && '✨ Dragging mode active'}
          </p>
        </div>
        {onTaskCreate && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-6">
        {GTD_LISTS.map((list) => {
          ...
          const isDraggedFromHere = draggedTask && draggedTask.status === list.id
          ...
          return (
            ...
            >
              <CardHeader className="pb-3">
                <CardTitle className={cn('flex items-center gap-2 text-lg', list.iconColor)}>
              ...
              <CardContent className="space-y-3">
                ...
                {listTasks.length === 0 && !isDropTarget ? (
                  <div className={cn(
                    ...
                    draggedTask && draggedTask.status !== list.id && 'border-gray-300 bg-gray-50'
                  )}>
                    <Icon className={cn('h-8 w-8 mx-auto mb-2 opacity-50', list.iconColor)} />
                    <p className="text-sm">No tasks in {list.title.toLowerCase()}</p>
                    {draggedTask && draggedTask.status !== list.id && (
                      <p className="text-xs mt-1">Drop tasks here to organize them</p>
                    )}
                  </div>
                ) : (
                ...
                {listTasks.length === 0 && onTaskCreate && list.id !== 'captured' && (
                  <Button
                    variant="ghost"
                    size="sm"
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-blue-900">GTD Organization Tips</h3>
                <p className="text-blue-700 text-sm mt-1">
                  Drag tasks between lists to organize them according to GTD principles.
                  Move captured items to Next Actions when they&apos;re clarified,
                  or to Projects if they require multiple steps.
                </p>
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-blue-600">
                  <div>📥 <strong>Inbox:</strong> New captures to process</div>
                  <div>⚡ <strong>Next Actions:</strong> Ready to do now</div>
                  <div>📁 <strong>Projects:</strong> Multi-step outcomes</div>
                  <div>⏳ <strong>Waiting For:</strong> Waiting on others</div>
                  <div>💭 <strong>Someday:</strong> Future possibilities</div>
                </div>
              </div>
            </div>
          </CardContent>
