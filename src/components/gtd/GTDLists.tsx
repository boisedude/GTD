...
export function GTDLists({ onTaskEdit, onTaskCreate, className }: GTDListsProps) {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (filters.context?.length) {
      filtered = filtered.filter(task => task.context && filters.context!.includes(task.context))
    }

    if (filters.energy_level?.length) {
      filtered = filtered.filter(task => task.energy_level && filters.energy_level!.includes(task.energy_level))
    }

    if (filters.due_today) {
      const today = new Date().toDateString()
      filtered = filtered.filter(task => task.due_date && new Date(task.due_date).toDateString() === today)
    }

    if (filters.overdue) {
      const now = new Date()
      filtered = filtered.filter(task => task.due_date && new Date(task.due_date) < now)
    }
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2">
            <Select value={selectedList} onValueChange={(value) => setSelectedList(value as TaskStatus | 'all')}>
          ...
          <Card className="p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-end">
                <Button
                  variant="outline"
                  onClick={() => setFilters(prev => ({ ...prev, due_today: !prev.due_today }))}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 mt-1">
                      {searchQuery ? 'Try adjusting your search or filters' : 'This list is empty'}
                    </p>
                  </div>
                </div>
              </CardContent>
