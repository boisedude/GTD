...
export function useProjects(
    ...
    async (id: string): Promise<void> => {
      try {
        ...
        if (tasks && tasks.length > 0) {
          throw new Error(
            `Cannot delete project with ${tasks.length} associated tasks. Please remove or reassign tasks first.`
          );
        }
  ...
  const getProjectTasks = useCallback(
    async (projectId: string): Promise<Task[]> => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to fetch project tasks";
        setError(errorMessage);
        throw err;
      }
    },
  ...
  const addTaskToProject = useCallback(
    async (projectId: string, taskInput: CreateTaskInput): Promise<Task> => {
      try {
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to add task to project";
        setError(errorMessage);
        throw err;
      }
    },
