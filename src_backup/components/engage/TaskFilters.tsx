...
const priorityOptions = [
  { value: 1, label: "Highest (1)", badgeVariant: "priority_high" as const },
  { value: 2, label: "High (2)", badgeVariant: "priority_high" as const },
  { value: 3, label: "Medium (3)", badgeVariant: "priority_medium" as const },
  { value: 4, label: "Low (4)", badgeVariant: "priority_low" as const },
  { value: 5, label: "Lowest (5)", badgeVariant: "priority_low" as const },
] as const;

// Status badge variants mapping for improved contrast
...
export function TaskFilters({
  ...
  const updateLocalFilter = (
    ...
    value: TaskFilter[keyof TaskFilter]
  ) => {
  ...
  const toggleArrayValue = <T,>(key: keyof TaskFilter, value: T) => {
    ...
    updateLocalFilter(
      key,
      newArray.length > 0
        ? (newArray as TaskFilter[keyof TaskFilter])
        : undefined
    );
  };
      ...
      <CardContent className="space-y-6">
            ...
            {statusOptions.map((option) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    toggleArrayValue("status", option.value as TaskStatus)
                  }
                ...
                >
                  <Icon
                    className={`h-4 w-4 mr-2 ${isSelected ? "text-white" : option.color}`}
                    aria-hidden="true"
                  />
                  {option.label}
                </Button>
        ...
        <fieldset className="space-y-3">
          ...
          <div
            className="grid grid-cols-2 gap-2"
            role="group"
            aria-labelledby="context-filter-label"
          >
            {contextOptions.map((option) => {
              const isSelected = localFilters.context?.includes(
                option.value as TaskContext
              );
              const Icon = option.icon;
              return (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    toggleArrayValue("context", option.value as TaskContext)
                  }
                  className="justify-start"
                  role="checkbox"
                  aria-checked={isSelected}
                  aria-label={`Filter by ${option.label} context`}
                >
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {energyOptions.map((option) => {
              const isSelected = localFilters.energy_level?.includes(
                option.value as TaskEnergyLevel
              );
              const Icon = option.icon;
              return (
                <Button
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {durationOptions.map((option) => {
              const isSelected = localFilters.estimated_duration?.includes(
                option.value as TaskDuration
              );
              return (
                <Button
                  key={option.value}
              ...
              return (
                ...
                >
                  <Badge
                    ...
                    className="w-3 h-3 rounded-full mr-2 p-0"
                  />
                  {option.label}
                </Button>
              );
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateLocalFilter("due_today", checked || undefined)
                }
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateLocalFilter("overdue", checked || undefined)
                }
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={
                  localFilters.has_project !== undefined
                    ? localFilters.has_project
                    : false
                }
                onCheckedChange={(checked) =>
                  updateLocalFilter("has_project", checked || undefined)
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4 border-t">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {localFilters.energy_level?.map((energy) => {
                const option = energyOptions.find(
                  (opt) => opt.value === energy
                );
                return (
