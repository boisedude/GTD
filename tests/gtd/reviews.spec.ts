...
import { TEST_USERS, REVIEW_WORKFLOWS, AI_COACHING_PROMPTS } from '../fixtures/test-data';
...
test.describe('GTD Review Workflows', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await helpers.captureTask('Waiting for feedback', { status: 'waiting_for' });
  ...
  test.describe('Daily Review', () => {
    test('should start daily review', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-header"]')).toContainText('Daily Review');
      await expect(page.locator('[data-testid="review-description"]')).toContainText('Quick daily check-in');
      await expect(page.locator('[data-testid="estimated-time"]')).toContainText('5-10 minutes');
      ...
      await expect(page.locator('[data-testid="total-steps"]')).toContainText('6'); // Based on REVIEW_WORKFLOWS
    });
    ...
    test('should complete daily review steps', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="step-title"]')).toContainText('Welcome');
      await helpers.completeReviewStep();

      // Step 2: Calendar Check
      await expect(page.locator('[data-testid="step-calendar-check"]')).toBeVisible();
      await expect(page.locator('[data-testid="step-title"]')).toContainText('Check Calendar');
      ...
      // Step 3: Task Triage
      await expect(page.locator('[data-testid="step-task-triage"]')).toBeVisible();
      await expect(page.locator('[data-testid="step-title"]')).toContainText('Review Today\'s Tasks');
      ...
      await expect(page.locator('[data-testid="step-title"]')).toContainText('Plan Tomorrow');

      // Should allow setting priorities for tomorrow
      await page.fill('[data-testid="tomorrow-priorities"]', 'Focus on urgent client work\nComplete project proposal');
      await helpers.completeReviewStep();

      // Step 5: Reflection
      await expect(page.locator('[data-testid="step-reflection"]')).toBeVisible();
      await expect(page.locator('[data-testid="step-title"]')).toContainText('Quick Reflection');

      // Should show reflection prompts
      await expect(page.locator('[data-testid="reflection-prompt"]')).toBeVisible();
      await page.fill('[data-testid="reflection-notes"]', 'Good progress today. Need to focus more on deep work tomorrow.');
      await helpers.completeReviewStep();

      // Step 6: Completion
      await expect(page.locator('[data-testid="step-completion"]')).toBeVisible();
      await expect(page.locator('[data-testid="review-summary"]')).toBeVisible();
      await helpers.completeReviewStep();

      // Should complete review and return to dashboard
      await expect(page).toHaveURL(/\/dashboard/);
      await expect(page.locator('[data-testid="review-completed-message"]')).toBeVisible();
    });
    ...
    test('should track daily review completion time', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-duration"]')).toContainText(/\d+ minutes?/);
    ...
    test('should pause and resume daily review', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="pause-message"]')).toContainText('Review paused');
    ...
    test('should handle daily review interruptions', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="continue-review-banner"]')).toBeVisible();
      await expect(page.locator('[data-testid="continue-review-banner"]')).toContainText('Continue Daily Review');
    ...
    test('should prevent multiple daily reviews', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="daily-review-completed"]')).toBeVisible();
      await expect(page.locator('[data-testid="daily-review-completed"]')).toContainText('Completed today');
    });
  });
  ...
  test.describe('Weekly Review', () => {
    test('should start weekly review', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-header"]')).toContainText('Weekly Review');
      await expect(page.locator('[data-testid="review-description"]')).toContainText('Comprehensive weekly review');
      await expect(page.locator('[data-testid="estimated-time"]')).toContainText('30-60 minutes');

      // Should show more comprehensive progress
      await expect(page.locator('[data-testid="review-progress"]')).toBeVisible();
      await expect(page.locator('[data-testid="total-steps"]')).toContainText('9'); // Based on REVIEW_WORKFLOWS
    });
    ...
    test('should complete weekly review workflow', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="step-calendar-check"]')).toBeVisible();
      await expect(page.locator('[data-testid="past-week-calendar"]')).toBeVisible();
      await helpers.completeReviewStep();

      // Step 3: Inbox Processing
      await expect(page.locator('[data-testid="step-inbox-process"]')).toBeVisible();
      await expect(page.locator('[data-testid="inbox-items"]')).toBeVisible();

      // Should show inbox items for processing
      await expect(page.locator('[data-testid="task-item"]:has-text("Inbox item 1")')).toBeVisible();
      await expect(page.locator('[data-testid="task-item"]:has-text("Inbox item 2")')).toBeVisible();
      ...
      for (let i = 0; i < count; i++) {
        ...
        await page.selectOption('[data-testid="clarify-action"]', 'next_action');
      ...
      await expect(page.locator('[data-testid="step-project-review"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="weekly-review-summary"]')).toBeVisible();
    });
    ...
    test('should show weekly insights and metrics', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="tasks-completed-count"]')).toBeVisible();
      await expect(page.locator('[data-testid="projects-progressed-count"]')).toBeVisible();
      await expect(page.locator('[data-testid="avg-tasks-per-day"]')).toBeVisible();
      await expect(page.locator('[data-testid="top-contexts"]')).toBeVisible();
      await expect(page.locator('[data-testid="productivity-streak"]')).toBeVisible();
    });
    ...
    test('should track weekly review performance', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="items-processed"]')).toBeVisible();
  ...
  test.describe('AI Coaching System', () => {
    test('should provide contextual coaching during reviews', async ({ page }) => {
      await helpers.startDailyReview();
      ...
      await expect(page.locator('[data-testid="coaching-prompt"]')).toBeVisible();

      // Should provide relevant suggestions
      const coachingText = await page.locator('[data-testid="coaching-prompt"]').textContent();
  ...
  test.describe('Review Analytics', () => {
    test('should track review completion rates', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-analytics"]')).toBeVisible();
      await expect(page.locator('[data-testid="daily-review-streak"]')).toBeVisible();
      await expect(page.locator('[data-testid="weekly-review-count"]')).toBeVisible();
      await expect(page.locator('[data-testid="avg-review-time"]')).toBeVisible();
    });
    ...
    test('should show productivity trends', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="completion-trends"]')).toBeVisible();
      await expect(page.locator('[data-testid="context-analysis"]')).toBeVisible();
    });
    ...
    test('should track review effectiveness', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-effectiveness"]')).toBeVisible();
      await expect(page.locator('[data-testid="items-clarified"]')).toBeVisible();
      await expect(page.locator('[data-testid="actions-identified"]')).toBeVisible();
    });
    ...
    test('should provide review recommendations', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-recommendations"]')).toBeVisible();
      await expect(page.locator('[data-testid="next-review-suggestion"]')).toBeVisible();

      // Recommendations should be actionable
      const recommendation = page.locator('[data-testid="recommendation-text"]');
  ...
  test.describe('Review Customization', () => {
    test('should allow customizing review steps', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="review-customization"]')).toBeVisible();
      await expect(page.locator('[data-testid="daily-review-steps"]')).toBeVisible();
      await expect(page.locator('[data-testid="weekly-review-steps"]')).toBeVisible();
    ...
    test('should support review templates', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="template-quick"]')).toBeVisible();
      await expect(page.locator('[data-testid="template-comprehensive"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="total-steps"]')).toContainText('3');
    });
    ...
    test('should remember review preferences', async ({ page }) => {
      ...
      // Set preferences
      await page.selectOption('[data-testid="preferred-review-time"]', 'morning');
      ...
      await expect(page.locator('[data-testid="morning-welcome"]')).toBeVisible();
    });
  });
  ...
  test.describe('Review Performance', () => {
    ...
    test('should handle step transitions smoothly', async ({ page }) => {
      ...
      expect(transitionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.REVIEW_STEP_MAX_TIME);
    });
