...
test.describe("GTD Organization", () => {
  ...
  test.describe("GTD Lists Navigation", () => {
    test("should navigate to inbox", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="list-description"]')
      ).toContainText("Capture and process items");
    });
    ...
    test("should navigate to next actions", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="list-description"]')
      ).toContainText("Ready to be acted upon");
    });
    ...
    test("should navigate to waiting for", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="list-description"]')
      ).toContainText("Waiting for others");
    });
    ...
    test("should navigate to someday maybe", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="list-description"]')
      ).toContainText("Things to do someday");
    });
    ...
    test("should navigate to projects", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="list-description"]')
      ).toContainText("Multi-step outcomes");
    });
    ...
    test("should show active navigation state", async ({ page }) => {
      ...
      ).toHaveClass(/active|current|selected/);

      // Other nav items should not be active
      await expect(page.locator('[data-testid="nav-inbox"]')).not.toHaveClass(
        /active|current|selected/
      );
      await expect(
        page.locator('[data-testid="nav-waiting-for"]')
      ).not.toHaveClass(/active|current|selected/);
    });

    test("should preserve navigation state on page reload", async ({
      page,
    }) => {
      await helpers.navigateToList("waiting-for");
      await page.reload();
      ...
      await expect(page).toHaveURL(/\/waiting-for/);
      await expect(page.locator('[data-testid="nav-waiting-for"]')).toHaveClass(
        /active|current|selected/
      );
    });
  });
  ...
  test.describe("Task Organization by Status", () => {
    test("should show tasks in appropriate lists", async ({ page }) => {
      ...
      await helpers.navigateToList("inbox");
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Inbox task")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Next action task")')
      ).not.toBeVisible();
      ...
      await helpers.navigateToList("waiting-for");
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Waiting task")')
      ).toBeVisible();

      // Check someday
      await helpers.navigateToList("someday");
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Someday task")')
      ).toBeVisible();
    });

    test("should move tasks between lists via drag and drop", async ({
      page,
    }) => {
      ...
      const taskItem = page.locator(
        '[data-testid="task-item"]:has-text("Drag and drop task")'
      );
      await expect(taskItem).toBeVisible();

      // Drag to next actions area (if drag-drop interface exists)
      const nextActionsDropZone = page.locator(
        '[data-testid="drop-zone-next-actions"]'
      );
    ...
    test("should support keyboard navigation for task movement", async ({
      ...
      const taskItem = page.locator(
        '[data-testid="task-item"]:has-text("Keyboard move task")'
      );
      ...
      await helpers.navigateToList("next-actions");
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Keyboard move task")')
      ).toBeVisible();
    });

    test("should provide visual feedback during task movement", async ({
      page,
    }) => {
      await page.goto("/dashboard");
      ...
      const taskItem = page.locator(
        '[data-testid="task-item"]:has-text("Visual feedback task")'
      );
  ...
  test.describe("Project Management", () => {
    test("should create new project", async ({ page }) => {
      ...
      await expect(
        page.locator(`[data-testid="project-item"]:has-text("${projectName}")`)
      ).toBeVisible();

      // Project should have default active status
      const projectItem = page.locator(
        `[data-testid="project-item"]:has-text("${projectName}")`
      );
      await expect(
        projectItem.locator('[data-testid="project-status"]')
    ...
    test("should show project tasks count", async ({ page }) => {
      ...
      await expect(
        projectItem.locator('[data-testid="project-task-count"]')
      ).toContainText("3");
    });
    ...
    test("should show project overview with task breakdown", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="next-actions-count"]')
      ).toContainText("1");
      await expect(
        page.locator('[data-testid="waiting-for-count"]')
      ).toContainText("1");
    });

    test("should handle project deletion with task reassignment", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="task-reassignment-modal"]')
      ).toBeVisible();
      ...
      await page.goto("/dashboard");
      const taskItem = page.locator(
        '[data-testid="task-item"]:has-text("Orphaned task")'
      );
      await expect(taskItem).toBeVisible();
      await expect(
        taskItem.locator('[data-testid="task-project-badge"]')
      ).not.toBeVisible();
    });
  });
  ...
  test.describe("Task Filtering", () => {
    test("should filter by context", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Office task")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Home task")')
      ).not.toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Call task")')
      ).not.toBeVisible();
    ...
    test("should filter by energy level", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("High energy task")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Low energy task")')
      ).not.toBeVisible();
    });
    ...
    test("should filter by estimated duration", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Quick task")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Long task")')
      ).not.toBeVisible();
    });

    test("should filter by tags", async ({ page }) => {
      await helpers.captureTask("Work task", { tags: ["work", "urgent"] });
      await helpers.captureTask("Personal task", {
        tags: ["personal", "home"],
      });
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Work task")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Personal task")')
      ).not.toBeVisible();
    });
    ...
    test("should combine multiple filters", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Office urgent task")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Office normal task")')
      ).not.toBeVisible();
    });

    test("should show filter count and active filters", async ({ page }) => {
      await helpers.captureTask("Filtered task", {
        context: "office",
        tags: ["work"],
      });
      ...
      await expect(
        page.locator('[data-testid="active-filter"]:has-text("office")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="active-filter"]:has-text("work")')
      ).toBeVisible();

      // Should show result count
      await expect(
        page.locator('[data-testid="filter-result-count"]')
    ...
    test("should save and restore filter preferences", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="active-filter"]:has-text("high")')
      ).toBeVisible();
    });
  });
  ...
  test.describe("Search Functionality", () => {
    test("should search tasks by title", async ({ page }) => {
      ...
      await expect(
        results.filter({ hasText: "Important meeting preparation" })
      ).toBeVisible();
      await expect(
        results.filter({ hasText: "Meeting room booking" })
      ).toBeVisible();
      await expect(
        results.filter({ hasText: "Grocery shopping list" })
      ).not.toBeVisible();
    });

    test("should search tasks by description", async ({ page }) => {
      await helpers.captureTask("Task 1", {
        description: "Prepare quarterly budget report",
      });
      await helpers.captureTask("Task 2", {
    ...
    test("should handle fuzzy search", async ({ page }) => {
      ...
      await expect(
        results.filter({ hasText: "Schedule appointment with dentist" })
      ).toBeVisible();
    });

    test("should search across all lists", async ({ page }) => {
      await helpers.captureTask("Inbox search task", { status: "captured" });
      await helpers.captureTask("Next action search task", {
        status: "next_action",
      });
      await helpers.captureTask("Waiting search task", {
        status: "waiting_for",
      });
      ...
      await expect(
        results.filter({ hasText: "Inbox search task" })
      ).toBeVisible();
      await expect(
        results.filter({ hasText: "Next action search task" })
      ).toBeVisible();
      await expect(
        results.filter({ hasText: "Waiting search task" })
      ).toBeVisible();
    });
    ...
    test("should highlight search terms", async ({ page }) => {
      ...
      const searchResult = page.locator(
        '[data-testid="task-item"]:has-text("Important deadline approaching")'
      );
      await expect(
        searchResult.locator('[data-testid="search-highlight"]')
    ...
    test("should show search suggestions", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="search-suggestion"]:has-text("email")')
      ).toBeVisible();
    });
  ...
  test.describe("List Management", () => {
    test("should show task counts for each list", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="nav-inbox"] [data-testid="task-count"]')
      ).toContainText("2");
      await expect(
        page.locator(
          '[data-testid="nav-next-actions"] [data-testid="task-count"]'
        )
      ).toContainText("1");
      await expect(
        page.locator(
          '[data-testid="nav-waiting-for"] [data-testid="task-count"]'
        )
      ).toContainText("0");
    });
    ...
    test("should support list-specific views", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="context-filter"]')
      ).toBeVisible();
      await expect(page.locator('[data-testid="energy-filter"]')).toBeVisible();
      await expect(
        page.locator('[data-testid="duration-filter"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="follow-up-date"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="waiting-for-who"]')
      ).toBeVisible();
    });
    ...
    test("should support bulk operations", async ({ page }) => {
      ...
      for (const task of tasks) {
        await page
          .locator(
            `[data-testid="task-item"]:has-text("${task}") [data-testid="task-checkbox"]`
          )
          .check();
      }
      ...
      for (const task of tasks) {
        await expect(
          page.locator(`[data-testid="task-item"]:has-text("${task}")`)
        ).toBeVisible();
      }
    });
  });
  ...
  test.describe("Mobile Organization Interface", () => {
    ...
    test("should support mobile-friendly interactions", async ({ page }) => {
      ...
      const taskItem = page.locator(
        '[data-testid="task-item"]:has-text("Mobile interaction task")'
      );
      ...
      await expect(
        page.locator('[data-testid="mobile-context-menu"]')
      ).toBeVisible();
    });
  });
});
