...
describe("useProjects", () => {
  ...
  describe("initialization", () => {
    ...
    it("should fetch projects on mount when autoRefresh is true", async () => {
      mockSupabase
        .from()
        .single.mockResolvedValue(mockSupabaseResponses.projects.select);
  ...
  describe("createProject", () => {
    ...
    it("should handle create project error", async () => {
      ...
      await act(async () => {
        await expect(
          result.current.createProject(projectInput)
  ...
  describe("updateProject", () => {
    it("should update a project successfully", async () => {
      ...
      await act(async () => {
        resultProject = await result.current.updateProject(
          "project-1",
          updateInput
        );
      });
    ...
    it("should handle update project error", async () => {
      ...
      await act(async () => {
        await expect(
          result.current.updateProject("project-1", updateInput)
        ).rejects.toThrow(errorMessage);
      });
  ...
  describe("getProjectTasks", () => {
    ...
    it("should handle get project tasks error", async () => {
      ...
      await act(async () => {
        await expect(
          result.current.getProjectTasks("project-1")
        ).rejects.toThrow(errorMessage);
      });
  ...
  describe("addTaskToProject", () => {
    it("should add a task to a project successfully", async () => {
      ...
      await act(async () => {
        createdTask = await result.current.addTaskToProject(
          "project-1",
          taskInput
        );
      });
    ...
    it("should handle add task to project error", async () => {
      ...
      await act(async () => {
        await expect(
          result.current.addTaskToProject("project-1", taskInput)
        ).rejects.toThrow(errorMessage);
      });
  ...
  describe("real-time sync", () => {
    ...
    it("should handle INSERT events from real-time subscription", () => {
      let subscriptionHandler: (payload: {
        eventType: string;
        new?: unknown;
        old?: unknown;
      }) => void;
