...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Application at ${baseURL} is not accessible. Please ensure the development server is running.`
      );
    } finally {
      await browser.close();
    }
    ...
    try {
      ...
      if (error && error.code !== "PGRST116") {
        // PGRST116 is "table not found" which is OK for setup
        throw error;
      }
      console.log("✅ Supabase connection verified");
    } catch (error) {
      console.warn(
        "⚠️ Supabase connection failed, tests may use mocks:",
        error
      );
    }
 ...
 */
async function setupTestSchema(supabase: SupabaseClient) {
 ...
 */
async function setupTestUsers(supabase: SupabaseClient) {
  const testUsers = [
 ...
 * Set up test data
 */
async function setupTestData(supabase: SupabaseClient) {
  try {
