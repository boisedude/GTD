...
test.describe("Task Capture", () => {
  ...
  test.describe("Quick Capture Interface", () => {
    ...
    test("should capture task with title only", async ({ page }) => {
      ...
      expect(captureTime).toBeLessThan(
    ...
    test("should capture task with detailed information", async ({ page }) => {
      ...
      const taskItem = page.locator(
        `[data-testid="task-item"]:has-text("${taskData.title}")`
      );
      await taskItem.click();

      await expect(
        page.locator('[data-testid="task-description"]')
      ).toContainText(taskData.description);
      await expect(page.locator('[data-testid="task-context"]')).toContainText(
        taskData.context
      );

      for (const tag of taskData.tags) {
        await expect(
          page.locator(`[data-testid="task-tag"]:has-text("${tag}")`)
        ).toBeVisible();
      }
    });
    ...
    test("should provide capture feedback", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="capture-success"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="capture-success"]')
      ).toContainText(/captured|added/i);
    ...
    test("should handle capture errors gracefully", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="capture-error"]')).toContainText(
        /error|failed/i
      );
  ...
  test.describe("Expanded Capture Modal", () => {
    test("should open expanded capture modal", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="task-tags-input"]')
      ).toBeVisible();
    });
    ...
    test("should capture task with all properties", async ({ page }) => {
      ...
      await page.selectOption(
        '[data-testid="task-context-select"]',
        taskData.context
      );
      await page.selectOption(
        '[data-testid="task-energy-select"]',
        taskData.energy_level
      );
      await page.selectOption(
      ...
      await taskItem.click();
      await expect(
        page.locator('[data-testid="task-detail-context"]')
      ).toContainText(taskData.context);
      await expect(
        page.locator('[data-testid="task-detail-energy"]')
      ).toContainText(taskData.energy_level);
      await expect(
        page.locator('[data-testid="task-detail-duration"]')
      ).toContainText(taskData.estimated_duration);
      await expect(
        page.locator('[data-testid="task-detail-priority"]')
      ).toContainText(taskData.priority.toString());
    ...
    test("should preserve partial input when modal is reopened", async ({
      page,
    }) => {
      await page.goto("/dashboard");
      await page.click('[data-testid="expand-capture"]');
      ...
      await expect(
        page.locator('[data-testid="task-title-input"]')
      ).toHaveValue("Partial task");
      await expect(
        page.locator('[data-testid="task-description-input"]')
      ).toHaveValue("Partial description");
    });
  });
...
Task five`;
      ...
      await expect(
        page.locator('[data-testid="bulk-capture-modal"]')
      ).toBeVisible();
      ...
      ).toContainText("5 tasks imported");

      // Verify tasks were created
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Task one")')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Task two")')
      ).toBeVisible();

      // Task with description should be parsed correctly
      const taskWithDesc = page.locator(
        '[data-testid="task-item"]:has-text("Task three")'
      );
      ...
      const taskWithTags = page.locator(
        '[data-testid="task-item"]:has-text("Task four")'
      );
  ...
  test.describe("Performance Testing", () => {
    test("should meet capture time requirements", async ({ page }) => {
      ...
      for (const scenario of scenarios) {
        const { captureTime } = await helpers.captureTask(
          scenario.title,
          scenario
        );
  ...
  test.describe("Voice Input (Future Feature)", () => {
    test.skip("should support voice-to-text capture", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="voice-recording"]')
      ).toBeVisible();
  ...
  test.describe("Smart Suggestions", () => {
    test("should suggest task properties based on content", async ({
      page,
    }) => {
      await page.goto("/dashboard");
      await page.click('[data-testid="expand-capture"]');
      ...
      await expect(
        page.locator('[data-testid="suggested-context"]')
      ).toContainText("calls");
      ...
      await expect(page.locator('[data-testid="suggested-tag"]')).toContainText(
        "urgent"
      );
    });
