...
test.describe("Authentication Flow Validation", () => {
  test("should load login page and handle auth state", async ({ page }) => {
    ...
    const loginButton = await page
      .locator('button:has-text("Send Code"), button:has-text("Login")')
      .count();
    ...
    const rebuildMessage = await page
      .locator("text=Environment Rebuilt")
      .count();
  ...
  test("should validate GTD workflow pages load", async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      const gtdContent = await page
        .locator("text=GTD, text=capture, text=clarify, text=organize")
        .count();
