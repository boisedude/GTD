...
test.describe('Engagement Interface', () => {
  ...
  test.describe('Engagement Dashboard', () => {
    test('should display engagement dashboard', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="engagement-dashboard"]')).toBeVisible();
      await expect(page.locator('[data-testid="what-to-do-next"]')).toBeVisible();
      await expect(page.locator('[data-testid="context-selector"]')).toBeVisible();
      await expect(page.locator('[data-testid="suggested-tasks"]')).toBeVisible();
    });
    ...
    test('should show current context information', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="current-context"]')).toBeVisible();
      await expect(page.locator('[data-testid="current-location"]')).toBeVisible();
      await expect(page.locator('[data-testid="available-time"]')).toBeVisible();
    ...
    test('should provide quick context switching', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="quick-context-switcher"]')).toBeVisible();

      // Test context switching
      await page.click('[data-testid="context-office"]');
      await expect(page.locator('[data-testid="current-location"]')).toContainText('office');

      await page.click('[data-testid="context-home"]');
      await expect(page.locator('[data-testid="current-location"]')).toContainText('home');

      await page.click('[data-testid="context-mobile"]');
      await expect(page.locator('[data-testid="current-location"]')).toContainText('mobile');
    });
    ...
    test('should update suggestions based on context', async ({ page }) => {
      ...
      await expect(suggestions.filter({ hasText: 'Write project proposal' })).toBeVisible();
      await expect(suggestions.filter({ hasText: 'Review team feedback' })).toBeVisible();
      ...
      await expect(mobileSuggestions.filter({ hasText: 'Call dentist' })).toBeVisible();
    });
  });

  test.describe('Task Recommendations', () => {
    test('should provide "What to do next" recommendations', async ({ page }) => {
      await helpers.goToEngagement();

      // Should show personalized recommendations
      await expect(page.locator('[data-testid="recommendation-header"]')).toContainText('What should you do next?');
      ...
      await expect(firstSuggestion.locator('[data-testid="task-title"]')).toBeVisible();
      await expect(firstSuggestion.locator('[data-testid="suggestion-reason"]')).toBeVisible();
      await expect(firstSuggestion.locator('[data-testid="suggestion-score"]')).toBeVisible();
    });
    ...
    test('should rank suggestions by relevance', async ({ page }) => {
      ...
      const firstSuggestion = suggestions.first();
      const firstScore = await firstSuggestion.locator('[data-testid="suggestion-score"]').textContent();

      const secondSuggestion = suggestions.nth(1);
      const secondScore = await secondSuggestion.locator('[data-testid="suggestion-score"]').textContent();
    ...
    test('should explain recommendation reasoning', async ({ page }) => {
      ...
      const reason = await firstSuggestion.locator('[data-testid="suggestion-reason"]').textContent();
    ...
    test('should update recommendations dynamically', async ({ page }) => {
      ...
      const initialFirstTask = await initialSuggestions.first().locator('[data-testid="task-title"]').textContent();
      ...
      const newFirstTask = await newSuggestions.first().locator('[data-testid="task-title"]').textContent();
    ...
    test('should handle no available tasks gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="no-tasks-message"]')).toContainText(/no tasks|all done|great job/i);

      // Should suggest review or capture
      await expect(page.locator('[data-testid="suggestion-capture"]')).toBeVisible();
      await expect(page.locator('[data-testid="suggestion-review"]')).toBeVisible();
    });
    ...
    test('should support task filtering in suggestions', async ({ page }) => {
      ...
      for (let i = 0; i < await suggestions.count(); i++) {
        ...
        await expect(suggestion.locator('[data-testid="task-priority"]')).toContainText('1');
      }
    });
  });
  ...
  test.describe('Context-Aware Filtering', () => {
    test('should filter tasks by location context', async ({ page }) => {
      ...
      await expect(suggestions.filter({ hasText: 'Buy groceries' })).not.toBeVisible();
    });
    ...
    test('should filter tasks by available time', async ({ page }) => {
      ...
      await expect(suggestions.filter({ hasText: 'Call dentist' })).toBeVisible();
      await expect(suggestions.filter({ hasText: 'Write project proposal' })).not.toBeVisible();
    });
    ...
    test('should filter tasks by energy level', async ({ page }) => {
      ...
      const highEnergyTask = suggestions.filter({ hasText: 'Write project proposal' });
      if (await highEnergyTask.count() > 0) {
        // If shown, should be ranked lower
        const allSuggestions = await suggestions.count();
        const highEnergyIndex = await suggestions.locator('text="Write project proposal"').count();
    ...
    test('should combine multiple context filters', async ({ page }) => {
      ...
      if (await suggestions.count() > 0) {
        const firstSuggestion = suggestions.first();
        await expect(firstSuggestion.locator('[data-testid="task-context"]')).toContainText(/(calls|anywhere)/);
        await expect(firstSuggestion.locator('[data-testid="task-duration"]')).toContainText(/(5min|15min)/);
        await expect(firstSuggestion.locator('[data-testid="task-energy"]')).toContainText(/(low|medium)/);
      }
    });
  ...
  test.describe('Task Timers and Focus Sessions', () => {
    test('should start timer for selected task', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="timer-task-title"]')).toContainText(taskTitle!);
      await expect(page.locator('[data-testid="timer-duration"]')).toBeVisible();
    ...
    test('should pause and resume timer', async ({ page }) => {
      ...
      const pausedTime = await page.locator('[data-testid="timer-duration"]').textContent();

      // Wait and verify time doesn't change
      await page.waitForTimeout(1000);
      const stillPausedTime = await page.locator('[data-testid="timer-duration"]').textContent();
    ...
    test('should complete task when timer stops', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="timer-completion-dialog"]')).toBeVisible();
      await expect(page.locator('[data-testid="complete-task-option"]')).toBeVisible();
      await expect(page.locator('[data-testid="continue-task-option"]')).toBeVisible();
    ...
    test('should save timer sessions', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="total-time-logged"]')).toBeVisible();
    });
    ...
    test('should support focus session modes', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="focus-mode-dialog"]')).toBeVisible();
      await expect(page.locator('[data-testid="pomodoro-mode"]')).toBeVisible();
      await expect(page.locator('[data-testid="deep-work-mode"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="focus-mode-active"]')).toContainText('Pomodoro');
      await expect(page.locator('[data-testid="timer-duration"]')).toContainText('25:00');
    });
  });
  ...
  test.describe('Quick Actions', () => {
    test('should provide quick task actions', async ({ page }) => {
      ...
      await expect(firstSuggestion.locator('[data-testid="quick-start"]')).toBeVisible();
      await expect(firstSuggestion.locator('[data-testid="quick-defer"]')).toBeVisible();
      await expect(firstSuggestion.locator('[data-testid="quick-complete"]')).toBeVisible();
    });
    ...
    test('should support quick task completion', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="quick-complete-confirmation"]')).toBeVisible();
      ...
      await expect(updatedSuggestions.filter({ hasText: taskTitle! })).not.toBeVisible();
    });
    ...
    test('should support quick task deferral', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="defer-later-today"]')).toBeVisible();
      await expect(page.locator('[data-testid="defer-tomorrow"]')).toBeVisible();
      await expect(page.locator('[data-testid="defer-next-week"]')).toBeVisible();
      ...
      const updatedSuggestions = await helpers.getTaskSuggestions();
      expect(await updatedSuggestions.count()).toBeLessThan(await suggestions.count());
    });
    ...
    test('should support swipe actions on mobile', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="swipe-complete"]')).toBeVisible();
  ...
  test.describe('Offline Action Queue', () => {
    ...
    test('should show queued actions status', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="pending-actions-count"]')).toContainText('2');
  ...
  test.describe('Performance and Accessibility', () => {
    ...
    test('should be keyboard accessible', async ({ page }) => {
      ...
      const hasTimer = await page.locator('[data-testid="timer-active"]').isVisible();
      const hasModal = await page.locator('[data-testid="task-detail-modal"]').isVisible();
    ...
    test('should handle large numbers of suggestions efficiently', async ({ page }) => {
      // Create many tasks
      for (let i = 0; i < 50; i++) {
