...
test.describe("End-to-End User Journeys", () => {
  ...
  test.describe("Complete GTD Workflow", () => {
    test("should complete full GTD workflow from capture to engagement", async ({
      page,
    }) => {
      ...
      for (const task of tasks) {
        await helpers.captureTask(task.title, {
          context: task.context as "office" | "home" | "calls" | "errands",
        });
      }
      ...
      await helpers.updateTaskStatus(
      ...
      await helpers.assignTaskToProject(
        "Gather Q4 metrics",
        "Q4 Quarterly Report"
      );
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("Call dentist")')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="review-completed"]')
      ).toBeVisible();
  ...
  test.describe("New User Onboarding Journey", () => {
    test("should guide new user through complete onboarding", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="tutorial-capture"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="tutorial-success"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="task-item"]:has-text("My second task")')
      ).toBeVisible();
  ...
  test.describe("Accessibility User Journey", () => {
    test("should be fully accessible via keyboard and screen reader", async ({
      page,
    }) => {
      ...
      const taskItem = page.locator(
        '[data-testid="task-item"]:has-text("Accessibility test task")'
      );
  ...
  test.describe("Performance Under Load Journey", () => {
    test("should maintain performance with realistic data load", async ({
      page,
    }) => {
